...
class RequestHandler(generic.RequestHandler):
  ...
  def do_GET(self):
    ...
    if len(self.path_info) > 1 and self.path_info[0] == 'lignes':
      self.send_trains()
  ...
  def send_ponctualite(self):
    ...
    filename = 'images/{}.png'.format('_'.join(['ponctualite'] + self.path_info[1:]))
    filepath = 'client/{}'.format(filename)
    html = '<img src="{}?{}" alt="ponctualite {}" width="100%">'.format(filename,random(),self.path_info[1])

    # si le fichier n'existe pas on le crée
    if ( not os.path.isfile(filepath) ):
      self.create_ponctualite(filepath)
  ...
  #
  # Création d'un graphique de ponctualité
  #
  def create_ponctualite(self,filepath):
